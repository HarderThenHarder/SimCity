...
class CitizenConfig:
    def __init__(self, scale, config, city_map):
        ...
        self.set_occupation("programmer", [0, 10], [49, 59])
        # 30 Accountants which index [ 10 ~ 24 + 60 ~ 74]
        self.set_occupation("accountant", [10, 25], [59, 74])
        # 20 Embedded Engineers which index [25 ~ 34 + 75 ~ 84]
        self.set_occupation("embedded engineer", [25, 35], [75, 85])
        # 5 Natural park staff which index [35 ~ 37 + 85 ~ 86]
        self.set_occupation("natural park staff", [35, 38], [85, 87])
        # 10 Market staff which index [38 ~ 42 + 87 ~ 91]
        self.set_occupation("market staff", [38, 43], [87, 92])
        # 3 Coffee staff which index [43 ~ 43, 92 ~ 93]
        self.set_occupation("coffee staff", [43, 44], [92, 94])
        # 3 Bar staff which index [44 ~ 44 + 94 ~ 95]
        self.set_occupation("bar staff", [44, 45], [94, 96])
        # 3 Library staff which index [45 ~ 45 + 96 ~ 97]
        self.set_occupation("library staff", [45, 46], [96, 98])
        # 4 Restaurant staff which index [46 ~ 47 + 98 ~ 99]
        self.set_occupation("restaurant staff", [46, 48], [98, 100])
        # 2 Cinema staff which index [48 ~ 49 + None]
        self.set_occupation("cinema staff", [48, 50], [])
    ...
    def set_occupation(self, occupation, index_list_1, index_list_2):
        if index_list_1:
        ...
        for i in range(len(citizen_index)):
            self.citizen_list[citizen_index[i]].set_occupation(occupation)
